...
# TODO: Change this to the quantile function of the
...
msft <- net.returns %>% dplyr::select(MSFT) %>% pull()
amd <- net.returns %>% dplyr::select(AMD) %>% pull()
aapl <- net.returns %>% dplyr::select(AAPL) %>% pull()
sbux <- net.returns %>% dplyr::select(SBUX) %>% pull()
itub<- net.returns %>% dplyr::select(ITUB) %>% pull()
fb <- net.returns %>% dplyr::select(FB) %>% pull()
nvda <- net.returns %>% dplyr::select(NVDA) %>% pull()
f <- net.returns %>% dplyr::select(F) %>% pull()
bac <- net.returns %>% dplyr::select(BAC) %>% pull()
t <- net.returns %>% dplyr::select(T) %>% pull()
xom <- net.returns %>% dplyr::select(XOM) %>% pull()
vale <- net.returns %>% dplyr::select(VALE) %>% pull()
...
fnorm = fitCopula(copula=normalCopula(dim=4),
                  data=data1,
                  method="ml")

ffrank = fitCopula(copula = frankCopula(3, dim = 4),
                   data = data1,
                   method = "ml")


fgumbel = fitCopula(copula = gumbelCopula(3, dim=4),
                    data = data1,
                    method = "ml")

fjoe = fitCopula(copula=joeCopula(2,dim=4),
                 data=data1,
                 method="ml")

ft = fitCopula(copula = tCopula(dim = 4),
                   data = data1,
                   method = "ml")
